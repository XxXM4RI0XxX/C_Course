...
int main()
{
    ...
    // &: Regresa la direccion en la memoria hacia la que se esta apuntando (en numeros enteros)
    ...
    p2->x=5; //p2->x == p1.x, El valor .x de la direccion hacia donde apunta p2, se le asigna el valor de 5
    p2->y=4;
    ...
    printf("%d",&p3->x); //Imprime la direccion en  la que se esta almacenando el valor x del puntero p3.
}